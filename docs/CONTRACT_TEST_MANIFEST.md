...
SceneExporter evaluates explicit project frames, ProgramStructuralFrameRenderer composes the active STRUCT placement, and final encoded output preserves window/fullscreen geometry, DEFAULT/CUSTOM chrome, dynamic `[frame]` copy, structural source-local frame identity, supported two-window MOSAIC split placement including horizontal-edge SPLIT:MAX with aspect-derived height until the vertical cap binds, ordinary split-window entry/exit motion plus deterministic split/non-split entry motion inside the existing window budget, pane-owned split CARD overlays, and split shell-cue suppression without bypassing recursive pane composition.
