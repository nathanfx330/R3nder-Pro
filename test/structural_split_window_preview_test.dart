...
''';
  ...
  testWidgets(
    'prearmed tail snapshot is already active on first close paint',
    (WidgetTester tester) async {
      const String source = '''[MOSAIC:wall]
[PANE:left]
[CLIP:red:red.mp4:0:0:12:1]
...
''';
      ...
      Widget preview({
        required int sourceFrame,
        required bool useCloseSnapshot,
      }) {
        return MaterialApp(
          home: SizedBox(
            width: 1280,
            ...
            child: StructuralSplitWindowPreview(
              ...
              fontFamily: 'monospace',
              chromeScale: 1.0,
              entryProgress: useCloseSnapshot ? 0.5 : 1.0,
              moving: true,
              useCloseSnapshot: useCloseSnapshot,
              backend: backend,
      ...
      // sourceDurationFrames is 12, so frame 9 is inside the final four-frame
      // pre-arm window while the split is still in normal showing mode.
      await tester.pumpWidget(
        preview(sourceFrame: 9, useCloseSnapshot: false),
      );
      for (int attempt = 0; attempt < 50 && !ready; attempt++) {
        await tester.runAsync(() async {
          await Future<void>.delayed(const Duration(milliseconds: 5));
      ...
      final StructuralSplitWindowPainter showing =
          tester
              .widget<CustomPaint>(
                find.byKey(
                  const ValueKey<String>('structural-split-window-frame'),
      ...
      final ui.Image liveLeft = showing.images[0]!;
      final ui.Image liveRight = showing.images[1]!;

      // Enter close at the authored final source frame. The first close build
      // must already use the detached tail snapshots; it must not wait for the
      // newly scheduled frame-11 render to finish.
      await tester.pumpWidget(
        preview(
          sourceFrame: placement.sourceDurationFrames - 1,
          useCloseSnapshot: true,
        ),
      );

      final StructuralSplitWindowPainter closing =
          tester
              .widget<CustomPaint>(
                find.byKey(
                  const ValueKey<String>('structural-split-window-frame'),
      ...
      expect(closing.images[0], isNotNull);
      expect(closing.images[1], isNotNull);
      expect(identical(closing.images[0], liveLeft), isFalse);
      expect(identical(closing.images[1], liveRight), isFalse);
    },
  );
