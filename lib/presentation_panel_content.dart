...
class PresentationPanelContent {
  ...
  /// Optional heading size in the 1920x1080 reference composition.
  ///
  /// This is not an output-pixel measurement. Preview and BAKE both multiply
  /// it by min(engineW / 1920, engineH / 1080).
  final double? headingSize;

  /// Optional body size in the 1920x1080 reference composition.
  final double? bodySize;

  /// Optional hero-image share of the card height, expressed as 0..1.
  ...
  for (int i = open + 1; i < close; i++) {
    ...
    switch (key) {
      ...
      case 'HEADING_SIZE':
        ...
        if (parsed == null || !parsed.isFinite || parsed <= 0.0) {
          preserveIssue(
            code: PresentationPanelIssueCode.malformedDirective,
            severity: PresentationPanelIssueSeverity.error,
            message: 'HEADING_SIZE requires a positive reference-unit number.',
            lineIndex: i,
            rawLine: rawLine,
          );
          break;
      ...
      case 'BODY_SIZE':
        ...
        if (parsed == null || !parsed.isFinite || parsed <= 0.0) {
          preserveIssue(
            code: PresentationPanelIssueCode.malformedDirective,
            severity: PresentationPanelIssueSeverity.error,
            message: 'BODY_SIZE requires a positive reference-unit number.',
            lineIndex: i,
            rawLine: rawLine,
          );
          break;
      ...
      case 'IMAGE':
        ...
        final String rawPercent =
            value.endsWith('%') ? value.substring(0, value.length - 1).trim() : '';
        final double? parsed = double.tryParse(rawPercent);
        if (parsed == null ||
            !parsed.isFinite ||
            parsed <= 0.0 ||
            parsed > 100.0) {
          preserveIssue(
            code: PresentationPanelIssueCode.malformedDirective,
            severity: PresentationPanelIssueSeverity.error,
            message: 'IMAGE requires a percentage greater than 0 and at most 100.',
            lineIndex: i,
            rawLine: rawLine,
          );
          break;
...
String formatPresentationPanelBody({
  ...
  if (headingSize != null) {
    out.writeln('HEADING_SIZE: ${_panelNumber(headingSize)}');
  }
  if (bodySize != null) {
    out.writeln('BODY_SIZE: ${_panelNumber(bodySize)}');
  }
  if (imageFraction != null) {
    out.writeln('IMAGE: ${_panelNumber(imageFraction * 100.0)}%');
  }
...
String _panelNumber(double value) {
  final int rounded = value.round();
  if ((value - rounded).abs() < 0.000001) return '$rounded';
  String out = value.toStringAsFixed(3);
