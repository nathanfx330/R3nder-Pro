...
# ExactClipSpeed gets a true const unity value for defaults and comparisons.
replace_once(
    "lib/edit_model.dart",
...
# Media conforming is separated from copying and owns the one source-span
# conversion used by both whole-file import and future in/out placement.
replace_once(
...
replace_between(
    ...
    "String _basename(",
    r"""int sourceSpanToProjectFrames({
...
# The import layer no longer needs its own rational-reduction implementation.
text = read("lib/edit_media_import.dart")
gcd_start = text.find("\nint _gcd(int a, int b) {")
if gcd_start < 0:
    raise SystemExit("lib/edit_media_import.dart: expected old _gcd helper")
write("lib/edit_media_import.dart", text[:gcd_start].rstrip() + "\n")

# Retire the empty-document CLIP bootstrap. addClip is the one CLIP writer.
replace_between(
...
replace_once(
    ...
    "speed ?? ExactClipSpeed(1)",
    "speed ?? ExactClipSpeed.unity",
)

# Shared placement layer. UI callers decide where; this layer authors one CLIP.
write(
    "lib/edit_media_placement.dart",
    r"""// ./lib/edit_media_placement.dart
//
...
# Workspace: V1 ADD VIDEO becomes append; V2 ADD OVERLAY retains its current
# playhead placement. Both share the same media authoring primitive.
replace_once(
...
replace_between(
    ...
    r"""  Future<void> _addVideo(String trackId) async {
...
replace_between(
    ...
    r"""  void _newEdit(EditDocumentModel model) {
...
# First-clip creation now exercises the shared placement API.
replace_once(
...
replace_between(
    ...
    r"""  test('first media placement creates EDIT main V1 and a real CLIP', () {
...
# Required exact source lengths for existing workspace fakes.
replace_once(
...
# Regression: V1 Add Video ignores a playhead parked inside the prior shot.
workspace_test = read("test/edit_workspace_test.dart")
marker = "  testWidgets('ADD OVERLAY creates V2 at current edit playhead',"
if workspace_test.count(marker) != 1:
    raise SystemExit("edit_workspace_test.dart: ADD OVERLAY marker is not unique")
new_widget_test = r"""  testWidgets('ADD VIDEO appends when playhead is inside existing V1 clip',
...
write(
    "test/edit_workspace_test.dart",
    workspace_test.replace(marker, new_widget_test + marker, 1),
)

# Media import tests prove exact source length and shared conversion.
replace_once(
...
media_import_test = read("test/edit_media_import_test.dart")
media_import_extra = r"""
...
"""
pos = media_import_test.rfind("\n}")
if pos < 0:
    raise SystemExit("edit_media_import_test.dart: no main closing brace")
write(
    "test/edit_media_import_test.dart",
    media_import_test[:pos] + media_import_extra + media_import_test[pos:],
)

# Focused placement contracts.
write(
    "test/edit_media_placement_test.dart",
    r"""// ./test/edit_media_placement_test.dart
...
# Journey record deliberately includes the failed invariant instead of making
# the architecture history look cleaner than it was.
write(
    "docs/MEDIA_BIN_PHASE0_JOURNEY.md",
    r"""# Media Bin Phase 0 Journey
